    ...
    class Login
    {
        ...
        public function index(): string
        {
            ...
            if (isset($_GET['status'])) {
                ...
                else if ($_GET['status'] === 'error_password'){
                ...
                else if($_GET['status'] === 'form_empty'){
                ...
                else if($_GET['status'] === 'user_not_found'){
        ...
        public function loginUser()
        {
            ...
            if($this->emptyInput($email, $password) == true){
                header("Location: /Php2/lab5/mvc_simple/login?status=form_empty");
                exit();
            }

            //xét email hợp lệ
            if($this->validEmail($email) == false){
                header("Location: /Php2/lab5/mvc_simple/login?status=invalid_email");
                exit();
            }
            ...
            if(empty($user) || !$user){
                header("Location: /Php2/lab5/mvc_simple/login?status=user_not_found");
                exit();
            }

            //kiểm tra mật khẩu
            if(!password_verify($password, $user['password'])){
                header("Location: /Php2/lab5/mvc_simple/login?status=error_password");
                exit();
            }

            //nếu đúng thì khởi tạo session
            $_SESSION['user'] = $user;
            header("Location: /Php2/lab5/mvc_simple/login?status=success");
            exit();
        }
        ...
        public function logout()
        {
            ...
            session_unset();
            session_destroy();
            header("Location: /Php2/lab5/mvc_simple/login?status=logout");
            exit();
        }
