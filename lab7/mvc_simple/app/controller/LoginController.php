    ...
    class LoginController
    {
        ...
        public function loginUser()
        {
            ...
            if($this->emptyInput($email, $password) == true){
                header("Location: /Php2/lab7/mvc_simple/login?status_login=form_empty");
                exit();
            }

            //xét email hợp lệ
            if($this->validEmail($email) == false){
                header("Location: /Php2/lab7/mvc_simple/login?status_login=invalid_email");
                exit();
            }
            ...
            if(empty($user) || !$user){
                header("Location: /Php2/lab7/mvc_simple/login?status_login=user_not_found");
                exit();
            }

            //kiểm tra mật khẩu
            if(!password_verify($password, $user['password'])){
                header("Location: /Php2/lab7/mvc_simple/login?status_login=error_password");
                exit();
            }

            //nếu đúng thì khởi tạo session
            $_SESSION['user'] = $user;
            header("Location: /Php2/lab7/mvc_simple/login?status_login=success");
            exit();
        }
        ...
        public function logout()
        {
            ...
            session_unset();
            session_destroy();
            header("Location: /Php2/lab7/mvc_simple/login?status_login=logout");
            exit();
        }
    }
