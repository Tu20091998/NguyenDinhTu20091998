    ...
    class RegisterController extends RegisterModel
    {
        ...
        public function registerUser(){
            ...
            if(isset($_POST["submit"]))
            {
                ...
                {
                    header("Location: /Php2/lab8/mvc_simple/register?status_register=form_empty");
                    exit();
                }
                ...
                {
                    header("Location: /Php2/lab8/mvc_simple/register?status_register=invalid_email");
                    exit();
                }
                ...
                {
                    header("Location: /Php2/lab8/mvc_simple/register?status_register=error_password");
                    exit();
                }

                //xét nếu email đã tồn tại
                if(RegisterModel::userExists($this->email) == true)
                {
                    header("Location: /Php2/lab8/mvc_simple/register?status_register=email_exists");
                    exit();
                }

                //nếu thoả mãn tất cả thì đăng ký người dùng
                $isRegistered = RegisterModel::insertUser($this->lastname, $this->firstname, $this->email, $this->password);

                //nếu đăng ký thành công
                if($isRegistered){
                    header("Location: /Php2/lab8/mvc_simple/register?status_register=success");
                    exit();
                }
                else{
                    header("Location: /Php2/lab8/mvc_simple/register?status_register=error");
                    exit();
                }
            }
        }
